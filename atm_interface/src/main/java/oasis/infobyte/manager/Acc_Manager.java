package oasis.infobyte.manager;

import oasis.infobyte.database.DatabaseConnection;
import oasis.infobyte.util.Date_Time;
...
public class Acc_Manager {
        ...
        try {
            ...
            if (acc_no != 0) {
                PreparedStatement preparedStatement =
                        connection.prepareStatement("SELECT * FROM accounts WHERE acc_no = ? and security_pin = ? ");
                ...
                if (resultSet.next()) {
                    ...
                    if (amount <= current_balance) {
                        ...
                        if (rowsAffected > 0) {
                            ...
                            String transaction =
                                    "INSERT INTO transactions(date, type, acc_no, amount, status, balance, security_pin) VALUES (?,?,?,?,?,?,?)";
                            ...
                            preparedStatement2.setString(5, "Successful");
    ...
    public void transfer_money(long sender_acc_no) throws SQLException {
        {
            assert connection != null;
            // List all users other than yourself
            PreparedStatement preparedStatement =
                    connection.prepareStatement("SELECT first_name,last_name,acc_no from users WHERE acc_no != ?");
            ...
            if (resultSet.next()) {
                ...
                System.out.format("+----------------------+------------+%n");
                do {
                    System.out.format(formatter, name, resultSet.getLong("acc_no"));
                    System.out.format("+----------------------+------------+%n");
                    ...
                    if (amount <= sender_current_balance) {
                        ...
                        if (rowsAffected1 > 0 && rowsAffected2 > 0) {
                            ...
                            String send_transaction =
                                    "INSERT INTO transactions(date, type, acc_no, amount, transfer_acc, status, balance, security_pin) VALUES (?,?,?,?,?,?,?,?)";
                            ...
                            sendpreparedStatement.setString(6, "Successful");
                            ...
                            // Receiver
                            PreparedStatement cpreparedStatement = connection.prepareStatement(
                                    "SELECT balance, security_pin FROM accounts WHERE acc_no = ?");
                            ...
                            String receive_transaction =
                                    "INSERT INTO transactions(date, type, acc_no, amount, transfer_acc, status, balance, security_pin) VALUES (?,?,?,?,?,?,?,?)";
                            PreparedStatement receivepreparedStatement = connection.prepareStatement(receive_transaction);
                            ...
                            receivepreparedStatement.setString(6, "Successful");
    ...
    public void getBalance(long acc_no) {
        ...
        try {
            assert connection != null;
            PreparedStatement preparedStatement =
                    connection.prepareStatement("SELECT balance FROM accounts WHERE acc_no = ? AND security_pin = ?");
            preparedStatement.setLong(1, acc_no);
            preparedStatement.setString(2, security_pin);
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet.next()) {
                ...
                // Table format
                String formatter = "| %-8s | %12s |%n";
                System.out.format("+----------+--------------+%n");
                System.out.format(formatter, "Balance", balance);
                System.out.format("+----------+--------------+%n");
            }
            else {
                System.out.println("Invalid Pin!");
    ...
    public void getTransactions(long acc_no) {
        ...
        try {
            assert connection != null;
            PreparedStatement preparedStatement =
                    connection.prepareStatement("SELECT * FROM transactions WHERE acc_no = ? AND security_pin = ?");
            preparedStatement.setLong(1, acc_no);
            preparedStatement.setString(2, security_pin);
